...
function cleanupComputationOfSingleSource(cell) {
	...
	cell.sourceSlot = -1;
	if (
		theSource.flags & 64 /* SingleObserver */ &&
		...
		theSource.observers[observerSlotOfLastSourceOfComputation] =
			lastObserverOfSource;
		theSource.observerSlots[observerSlotOfLastSourceOfComputation] =
		...
		if (lastObserverOfSource.flags & 128 /* SingleSource */) {
			...
			lastObserverOfSource.sourceSlots[sourceSlotOfLastObserverOfSource] =
	...
	while (cell.sources.length > remain) {
		...
		if (
			...
			theSource.observerSlots[observerSlotOfLastSourceOfComputation] =
				sourceSlotOfLastObserverOfSource;
			if (lastObserverOfSource.flags & 128 /* SingleSource */) {
				lastObserverOfSource.sourceSlot =
					observerSlotOfLastSourceOfComputation;
			} else {
				lastObserverOfSource.sourceSlots[sourceSlotOfLastObserverOfSource] =
					observerSlotOfLastSourceOfComputation;
