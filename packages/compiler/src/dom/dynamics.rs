...
impl<'a> AstDomTransform<'a, '_> {
    ...
    /// Patch-mode emission (Babel's `wrapPatchMode`, DESIGN-PATCH-CHANNEL
    /// PR-C): one compiled body doing inline compares + setAttr writes,
    /// handed to the runtime driver which registers on the store patch
    /// channel (patchable subject) or falls back to a tracked force-mode
    /// effect running the SAME body. Returns None when the scope is
    /// ineligible — caller falls through to the effect shapes. The subject
    /// rides along for row-proof analysis.
    ///
    ///   _$patchDriver(subject, (_n$, _p$, _f$) => {
    ///     const _v$ = <expr(_n$)>;
    ///     if (_f$ || _v$ !== <expr(_p$)>) <setAttr(..., _v$)>;
    ///     ...
    ///   })
    pub(crate) fn wrap_patch_mode_statement(
        &mut self,
        dynamics: &[DynamicSlot<'a>],
    ) -> Option<(Statement<'a>, String)> {
        let driver = self.patch_driver.clone()?;
        if dynamics.is_empty() {
            return None;
        }
        let values: std::vec::Vec<&Expression<'a>> =
            dynamics.iter().map(|slot| &slot.value).collect();
        let subject = crate::shared::patch::analyze_patch_eligibility(&values)?;
        // The subject must be stable (Babel checked `binding.constant`): the
        // effect fallback re-evaluates the subject reference per run, but a
        // registered patch captures it once — a reassignable subject must
        // keep effect semantics.
        if !self.bindings.has_binding(&subject) || self.bindings.is_reassigned(&subject) {
            return None;
        }
        let span = dynamics
            .first()
        ...
        for (index, slot) in dynamics.iter().enumerate() {
            let slot_span = slot.span;
            let cloned = slot.value.clone_in(self.allocator);
            let value = if slot.class_property
                && !matches!(
                    slot.value,
                    Expression::BooleanLiteral(_) | Expression::UnaryExpression(_)
                ) {
                self.double_negation(slot_span, cloned)
            } else {
                cloned
            };
            let v_name = if index == 0 {
                "_v$".to_string()
            } else {
                format!("_v${}", index + 1)
            };
            let n_value =
                crate::shared::patch::substitute_subject(self.allocator, &value, &subject, "_n$");
            let p_value =
                crate::shared::patch::substitute_subject(self.allocator, &value, &subject, "_p$");
            ...
            let compare = self.ast().expression_binary(
                slot_span,
                self.identifier_expression(slot_span, &v_name),
                BinaryOperator::StrictInequality,
                p_value,
            );
            ...
            statements.push(self.ast().statement_if(
                slot_span,
                test,
                self.ast().statement_expression(slot_span, set_attr),
                None,
            ));
        }
