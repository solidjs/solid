...
`Errored`'s `on` prop (and `createErrorBoundary`'s `on` option) is removed. A retry is a recompute of the failed sources, and a recompute under the frame that carried the `on` change stages the recovered content with that frame instead of retrying now; keeping it would take a second drain point outside the frame that nothing else needs. Use the `reset` passed to the fallback (`fallback={(err, reset) => ...}`), or re-mount the boundary on the dependency (`<Show keyed when={id()}>`) — the same recovery as before the prop existed.
