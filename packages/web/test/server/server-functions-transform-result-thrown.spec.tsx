/**
 * `transformResult` and the failures it never sees.
 *
 ...
 *
 * Dispatch honors that on the return path and on one half of the throw
 * path: a thrown `Response` or `ResponseEnvelope` is offered to the hook,
 * because the tail that handles those calls it. The plain-thrown tail —
 * `respondThrown`, where a thrown `Error` or a thrown string goes — never
 * does. So the hook sees every SUCCESS and every failure an author already
 ...
 * written for. Silently: nothing logs, and the 500 looks the same as it
 * would with no hook at all.
 *
 * Either the code is wrong or the doc is. This spec takes the position that
 * the code is: a hook whose whole purpose is result policy cannot be
 * exempt from the majority of results, and `thrown` is already carried
 * into the context of the half that works. If the fix goes the other way —
 * narrowing the doc to "returned results and thrown HTTP shapes" — these
 * tests go with it.
 *
 ...
 * sanitize what it returns cannot go green here.
 *
