...
The kept tail carries writes, not pending marks (#3494 review; fuzzer latest-1 #2141). The tail is kept so that a _write_ to a dependency the committed frame still derives from reaches the node, whose pass then decides what it reads. A source going _pending_ behind that tail is different: it is a flight the node's next frame never asked for, and a pending mark is a claim on the node's transaction — the node registers as a reporter of the flight (A15) and the hold waits on it. Before: `show` written in the same tick as an action's proposal, then hidden from mainline — the hide rides with the action (A34) and its pass under the parked transaction stops reading the memo, unchanged, tail kept; the action's truth re-asks the memo, the pending mark rides the kept link back to the hidden reader, and the truth waits for a fetch nobody displays. Now a pending mark rides only the links the pass made; the node's own read of a pending source still registers it (the read links before it throws), and mid-pass a dependency the pass has yet to reach registers through that read. Clears and errors are unchanged: they ride every link, so a reader whose kept tail still points at a landing flight learns of the landing.
...
### O4. Adopted, unflushed — the signal's verdict channels see a write no flush has carried — fixed

**Status:** **violation, fixed** 2026-09-16 — posture matrix over the store states beside the signal states (`staged, ambient × foreignAction`): the store leaf answers `latest` 0 / `isPending` false inside the adopting action's body; the signal answers 1 / true. Pinned `it.fails` (signal) beside the passing store twin in `tests/posture-store-parity.test.ts` (S1).
...
**Was:** adoption stamps the signal with the transaction (`initTransition`'s pending-node loop); `unflushedValue` read a stamped node with no `_flushedStaged` stash as a flushed held node and served `_pendingValue`. The store's selection happened to answer by the foreign-hold rule instead. One rule, two implementations.
**Mechanism (2026-09-16):** one definition — a node staged outside a flush and adopted before any flush has carried it is unflushed whatever its stamp. `initTransition`'s adoption loop sets `CONFIG_ADOPTED_UNFLUSHED` when it runs outside a flush; `unflushedValue` serves the committed value for it (so `latest()` answers the pre-write value) and `flushedStaged` answers NOT_PENDING (so the verdict sees nothing staged); `reassignPendingTransition` — the carrying flush re-stamping the transaction's pending nodes — clears the bit, and the hold takes over. Signal and store leaves now answer through the same arm; `tests/posture-store-parity.test.ts` S1 pins both. (A first cut keyed on the transaction's `_time === clock`; re-entry re-stamps `_time`, so a genuinely held value read from another action read as unflushed — four pins caught it.) +56 B minified.
