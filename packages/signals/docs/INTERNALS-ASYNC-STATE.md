...
- `(held value, active value)` **+ `CONFIG_OVERRIDE_SUPERSEDED`** (#3331, A18 supersession) — the
  ...
  the scheduler's `origin` at the optimistic write): `origin` is the invocation sequence of the
...
- Reveal-hold (#3305, #3334): a render reader landing on a node with `STATUS_PENDING` **throws regardless of the node's `_transition` stamp** (`read`'s pending branch has no stale/foreign-transaction carve-out). The staged-_value_ rule above is separate and stands: a stale mainline reader sees a foreign transaction's staged signal as committed, but an in-flight async is never served as its pre-flight value — the stamp is pending-node bookkeeping and does not mean that transaction also holds the flight's inputs (they may be committed, lane-revealed, or held only by another reveal waiting on the same flight). The throw reaches `GlobalQueue.notify`, which opens a transaction for the reveal if none is active (#3305) and records the source as its reporter (INV-3); the reveal completes when the flight lands.
