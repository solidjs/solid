// Node harness for the sync-SSR (renderToString) mode. Dev runs Vite in
// middleware mode and SSR-loads ./entry-server.tsx; prod serves the built
...
async function start() {
  ...
  const server = createHttpServer(async (req, res) => {
    ...
    try {
      ...
      if (!isProduction) {
        ...
        html = html.replace("/client.tsx", getClientEntry());
      }
  ...
  server.listen(port, () => {
    console.log(`String SSR example listening on http://localhost:${port}`);
  });
}
