...
**`deferStream: true`** defers the SSR stream flush until this primitive's first value has resolved. It lets a late-resolving source hold the document open rather than forcing the surrounding `<Loading>` boundary to render its fallback into the HTML. The primitive must be created before the initial shell flush; a source created later (for example, after a lazy-loaded route resolves) streams normally and emits a development warning. Server-only; ignored on the client.
