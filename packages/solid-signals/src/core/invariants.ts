...
export function devCensusCompanions(isQueuedForCommit?: (node: AnyNode) => boolean): void {
  ...
  for (const node of companionOwners) {
    ...
    const hasOverride = node._x?._overrideValue !== undefined && node._x?._overrideValue !== NOT_PENDING;
    ...
    if (pendingSignal) {
      ...
      const cached =
        pendingSignal._x?._overrideValue !== undefined && pendingSignal._x?._overrideValue !== NOT_PENDING
