...
describe("pay-for-use tree-shaking (#2883)", () => {
  it("core floor sheds every optional feature module", async () => {
    ...
    // re-arms it for the landing's propagation. On the A28 write path the
    // landing's supersession decision runs at the write's promotion
    // (promoteUnflushed's override arm hands the node to the hook, still
    // under the flight's provenance — the landing flushes synchronously).
    // Core-retained by necessity: read visibility, the landing branch, the
