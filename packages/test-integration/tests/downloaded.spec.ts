...
function npmInstaller(dependencies: string[]) {
	return dependencies
		.map(dep => `npm install --save "${dep}" --ignore-scripts`)
		.join(" && ");
}
...
describe("Downloaded tests", () => {
	...
	let packedSolidPkg: string,
		packedBabelSolidPkg: string,
		dependencies: string[];
	beforeAll(() => {
		...
		packedBabelSolidPkg = pack(
			resolve(__dirname, "../../babel-preset-solid")
		);
