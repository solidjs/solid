...
- In dev, the component wrapper stores a `console.createTask(label)` task on the component record (`_component.task`), and every span and marker is emitted inside the nearest component's task, so the entry's stack in the panel points at the JSX site that rendered the component.
