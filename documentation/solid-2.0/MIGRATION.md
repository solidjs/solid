...
- **`Loading` / `Errored` `on` prop** — a dependency list for the boundary: a tracked expression whose value is irrelevant. A change to anything it reads re-arms the boundary — `Loading` shows its fallback again now, in the current frame (if something under it is still pending), instead of keeping stale content through revalidation; `Errored` clears its error and retries the children.
