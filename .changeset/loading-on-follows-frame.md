...
If the same data the boundary is waiting on is also read outside it, the frame waits on that read and no fallback appears; DEV warns `LOADING_ON_OUTSIDE_HOLD` with the fix (read `latest()` in `on`, or move the outside read under the boundary). `Errored`'s `on` retry is unaffected: it runs mainline — content now, the action's other writes later.
