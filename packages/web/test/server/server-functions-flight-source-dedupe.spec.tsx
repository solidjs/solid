/**
 * The request-leg source list is attacker-controlled input, and a collector
 * is the most expensive per-request work single-flight does.
 *
 ...
 * every run past the first is work whose result is thrown away.
 *
 * Nothing dedupes it, so a same-origin authenticated caller who repeats one
 * id N times pays for one request and gets N collections — each a re-run of
 * the invalidated reads inside a request-event scope — plus an echoed
 * response header N ids long. That is amplification with a multiplier the
 * caller chooses, on the single most expensive thing in the request.
 *
 * Deduping is also the only reading consistent with the protocol as
 * documented: the id list is "the ids its registered consumers can actually
 * use", and the response header "names the folded sources".
 *
