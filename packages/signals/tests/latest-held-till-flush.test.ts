...
describe("latest() is held till flush", () => {
  ...
  // latest() is an optimistic view: a derivation over it publishes the way an
  // optimistic correction does (direct commit), whatever transaction the round
  // entered. The shadow companion created by this first latest() read did not
  // exist at the write, so the write-time sync missed it; it joins the
  // flush-start re-sync instead (joinUnflushedResync) and is brought current
  // by the optimistic write — not by a plain recompute, which would have
  // staged it, and the memo over it, under the hold `data` re-enters (the memo
  // then answered 20 until the fetch settled).
  ...
  it("an effect over latest() created after an unflushed rewrite of a held node runs at flush (not parked with the hold)", async () => {
    const { count, setCount } = await heldSetup(10);
    setCount(20);
    flush();
    setCount(30);
