 ...
 * only when `JSON.stringify` carries it FAITHFULLY (#3253). That is the
 * whole contract of `isJSONSafe` (server-functions/shared.js), which both
 * peers consult — the client for argument lists, the server for results —
 * so the codec rides the wire exactly when a value actually needs it.
 *
 * `-0` is the one number that breaks the contract quietly.
 * `JSON.stringify(-0)` is `"0"`, but the guard admitted any
 * `Number.isFinite(v)`, so a signed zero took the fast path and arrived as
 * `+0`. Status 200, the function runs, the sign is simply gone — the
 * failure mode the guard's own siblings were written to prevent
 * (`undefined` corrupted to `null`, a sparse hole read back as `null`).
 *
 * `NaN` and the infinities, the other numbers stringify cannot carry, are
 * already refused and ride the codec instead — and the codec encodes `-0`
 * exactly, as its own constant. Each test pairs its case with the `NaN`
 * control that already behaves, so a reader can see the two halves agree.
 *
 * The captured requests are the encode-side assertion: which format tag the
 * negotiation picked, and what bytes actually rode the wire.
 *
...
describe("a signed zero on the wire (#3253)", () => {
  it("refuses the JSON fast path on the encode side, the way NaN already does", async () => {
    registerServerFunction("negative-zero-encoding", async (n: number) => Object.is(n, -0));
    const transport = connectTransport();
    try {
      await createServerReference("negative-zero-encoding")(NaN);
      await createServerReference("negative-zero-encoding")(-0);
      const [nanRequest, zeroRequest] = transport.requests;
      const control = nanRequest.headers.get(BODY_FORMAT_HEADER);
      expect(control, "the NaN control changed roads — rebaseline this spec").not.toBe(JSON_FORMAT);
      expect(
        zeroRequest.headers.get(BODY_FORMAT_HEADER),
        `-0 rode format ${zeroRequest.headers.get(BODY_FORMAT_HEADER)} ` +
          `as ${await zeroRequest.clone().text()} while NaN rode ${control}`
      ).toBe(control);
    } finally {
      transport.restore();
    }
  ...
  it("keeps its sign inside an otherwise JSON-safe object result", async () => {
    ...
    // the rest onto the codec: with NaN alongside it the -0 survives either
    // way, and without it the same field used to be flattened. Same data,
    // same shape — the company it keeps must not decide whether the sign
    // lives.
