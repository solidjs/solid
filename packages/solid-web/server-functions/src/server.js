...
import { REVALIDATE_HEADER, isResponseEnvelope, isSafeError } from "../../src/response.ts";
import { RequestContext, commitEventResponse, getRequestEvent } from "../../server/server.ts";
...
export function serializeResponseStream(value, codecOptions, signal) {
  ...
  return new ReadableStream({
    ...
    async start(controller) {
      ...
      const { serializeJSON } = await import("@solidjs/web/serialization");
      if (closed) {
