...
class DataNode {
	...
	next(nextValue) {
		...
		for (const v of this._runObservers.values())
			!v._fresh && updateComputation(v);
...
function _unsubscribe(node) {
	let i;
	for (i = 0; i < node._children.length; i += 1)
		_unsubscribe(node._children[i]);
...
function updateComputation(node) {
	...
	for (let i = 0; i < allChildren.length; i += 1)
		removeFreshChildren(allChildren[i]);
...
function currentValue() {
	if (this._fresh) {
		for (let i = 0; i < this._observables.length; i += 1)
			this._observables[i].current();
	} else {
