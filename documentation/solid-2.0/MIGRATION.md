...
- **`Loading` `on` prop** — a key for the boundary (a keyed `<Show>` around it, minus the remount): a write that changes it shows the fallback again with that write, instead of keeping stale content through revalidation.
