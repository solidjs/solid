...
export default class Handler
  ...
  get: (target, property) ->
    ...
    if property.endsWith('$')
      property = property[...-1]
      if Core.context.fn
        target[property] or= {_subs: new Set()}
    ...
    return @_state[property] if not Core.context?.fn or property is 'length' or typeof property is 'symbol'
    if (value = @_state[property])? and Core.isObject(value) and not (value instanceof Element)
      ...
      value = new Proxy(target[property], new Handler(target[property], value, @root))
  ...
  peek: (target, property) ->
    ...
    target[property] or= {}
    target[property].path or= target.path.concat([property])
    new Proxy(target[property], new Handler(target[property], value, @root))
  ...
  map: (target, fn) ->
    ...
    @_state.map (value, i, a) =>
      unless not value? or Core.isFunction(value) or not Core.isObject(value) or (value instanceof Element)
        ...
        value = new Proxy(target[i], new Handler(target[i], value, @root))
      fn(value, i, a)
