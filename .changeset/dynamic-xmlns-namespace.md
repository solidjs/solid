...
Types: `xmlns` is now accepted on every element (`ElementAttributes`), not only SVG/MathML ones — the compiler has honored `<a xmlns=…>` on any tag all along, and `dynamic()`'s tag-name components inherit it through their intrinsic attribute types. Also syncs `@solidjs/h`'s generated JSX types.
