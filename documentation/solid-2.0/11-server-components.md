...
5. **Server mutation policy ahead of persistence** — recorded now so the line is drawn before persistent server components exist. The rule is **blanket: no server write is legitimate.** Server render is pure — change enters server state only through async sources and RPC-mediated mutation (server functions writing to the actual source of truth, state re-derived), never through setter calls. No ownership-scoped carve-out is needed: discriminating writes by owning scope is already the _client_ dev guards’ job, and isomorphic code that misplaces a write screams there regardless of which side it runs on. Today setter writes are tolerated as _data-only_ operations (they update state for subsequent reads and serialization; nothing re-renders — see #3064), with optimistic writes hard no-ops since server output is settled state — but that tolerance is transitional, not contractual. The enforcement is staged: this release ships a `[SERVER_WRITE]` deprecation warning on every setter category (signal, store, optimistic; once per process per category) in all server builds, giving the ecosystem a loud migration window; the throw follows once it clears. The audit backing the stage plan: the runtime itself never writes through public setters (verified by making them throw under the full test surface — zero scenario failures), solid-router has two small fixes (the redirect `setSource` becomes a plain box, the lazy-tree version bump gets a server guard), the solid-query rewrite needs `isServer` guards on its three observer hooks, and TanStack Router/Start are already clean — their server store factory hands out non-reactive plain boxes by construction. The guard should land before any server scope can outlive a request, where an in-place write stops being an impurity and becomes a cross-request data race or cross-user leak. The concrete shape to reject: long-lived shared state fed by a subscription callback (`subscription.on(msg => setStore(...))`) — idiomatic on the client, a data race on the server. The subscription must instead _be_ the async source (`createStore(asyncIterable)`, derivations over it), so change is pulled through derivation rather than pushed by whatever thread the callback fires on. The rc-era habit of “setters work on the server” must not be allowed to generalize.
