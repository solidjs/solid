...
export function createResource<T, S>(
	...
	return [
		read as Resource<T>,
		{ refetch: load, mutate: set } as ResourceActions<T>
	];
}
 ...
 */
export function on<
	S extends Accessor<unknown> | Accessor<unknown>[] | [],
	Next,
	Init = unknown
>(
	deps: S,
...
export function devComponent<T>(Comp: (props: T) => JSX.Element, props: T) {
	const c: Partial<Memo<JSX.Element, JSX.Element>> = createComputation<
		JSX.Element,
		JSX.Element
	>(
		() =>
...
export type ContextProviderComponent<T> = (props: {
	value: T;
	children: any;
}) => any;
...
export type ResolvedJSXElement = Exclude<
	JSX.Element,
	JSX.ArrayElement | JSX.FunctionElement
>;
...
export function getSuspenseContext() {
	return (
		SuspenseContext ||
		(SuspenseContext = createContext<SuspenseContextType>({}))
	);
}
