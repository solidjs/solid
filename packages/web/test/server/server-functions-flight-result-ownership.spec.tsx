/**
 * The single-flight fold must own a `transformFlightResult` Response before
 * stamping it (#3234, completing #3155).
 *
 * `transformFlightResult` is the seam where an integration builds the
 * single-flight body itself, and nothing in its contract says the Response
 * it returns is freshly constructed per call. `foldFlightData` appends the
 * mutation's `Set-Cookie`s and gap-fills the accumulated headers onto
 * whatever came back — writes that land permanently on an object the
 * integration may still hold (a memoized shell), serving one tenant's
 * session cookie to the next. The thrown path's fold tail already copies
 * for exactly this reason ("the fold may hand back a Response an
 * integration hook caches (see ownResponse)"), but its copy is taken AFTER
 * the fold's writes have landed on the shared object.
 *
 * Stock Solid's frames policy constructs a fresh Response per call, so
 * these specs pin the contract for the seam as documented, which any
 * integration may implement.
 *
