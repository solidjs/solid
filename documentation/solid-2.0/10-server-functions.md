...
- **`createEvent(request)`** — build the request event a call runs under; integrations supply their richer event (cookies, response helpers, platform handles). The `request` handed here is a **standards-shaped `Request` and nothing more** — url, method, headers, signal, readable body. Enforcing `bodySizeLimit` puts the runtime between the host's stream and the decoder, so this may be a `Request` the runtime rebuilt, and host-specific fields on the inbound object (`request.cf`, srvx's `runtime`/`ip`/`waitUntil`) are not carried. This is a ruling, not a gap: any body-size enforcement on a Fetch body forces a rebuild, and `event.request` follows Solid's standards-only stance everywhere else. The host has its own request in closure when it calls the handler — read platform handles there and put them on the event (`locals`), which is where integrations should surface them regardless.
...
**Observing invocations (dev and observe builds).** Both legs — HTTP dispatch and direct SSR calls — report each execution as the `"invocation"` record on `OBSERVE.records`, the records channel described in [RFC 08](08-dev-diagnostics.md#observerecords--the-runtimes-records-channel): `subscribe("invocation", (event, live) => …)` delivers one `{ id, direct, at, durationMs, outcome, deferred?, boundary? }` record per call when it settles, with the request event, `request` (HTTP only), `args`, and the result or the error **as thrown** beside it. A direct call made during a `<Loading>` boundary's render pass carries that boundary's hydration id in `boundary` — the `"boundary"` record's `id` — so a boundary's wait reads as the calls it consisted of. The measured execution is the `wrapInvocation`-wrapped run, so what the observer times is what the request spent on the call, policy included; a stream or generator result is flagged `deferred` and timed to handoff. Observers, any number, none able to change the call. Prod server artifacts carry neither the channel nor the emit sites.

**Observing calls (dev and observe builds).** The client reports each call it makes as the `"call"` record on the same channel — `subscribe("call", (event, live) => …)` delivers one `{ id, at, durationMs, method, outcome, status?, origin?, deferred? }` per call when the caller's await settles, with `args`, the `response` (once one arrived) and the result or the error as thrown to the caller beside it. `durationMs` is the whole wait the caller saw — request, response, decode (or the `responseHandler`'s claim) — so against the server's `"invocation"` of the same `id` the difference is the wire. `origin` is what the call ran for when the attribution engine is enabled — the interaction whose handler made it, the navigation whose data needed it — as the engine's own origin object (RFC 08), so an observer files the call under that interaction's record by identity. The record rides the observe and dev artifacts of `@solidjs/web/server-functions/client` (the `observe` / `development` export conditions) and folds out of prod; a call an integration answered locally (`intercept`) made no request and emits nothing. There is no other client observation hook: the `fetch` option is for code that _sends_ differently, `prepareRequest` for code that _changes_ the request. A listener that wants the trace a call belongs to reads `getTraceContext()` — in scope during delivery — and a function body forwards it downstream the same way (`getTraceContext()?.entries.traceparent`); the handler’s responses carry it back to the caller as `Server-Timing` (see [RFC 12](12-ssr-http.md#the-trace-the-request-belongs-to-gettracecontext)).
...
- **Server:** the `collectFlightData(event, outcome)` hook (config or per-handler) receives the request event and a `ServerFunctionOutcome` — `{ id, value, response, request, thrown }` — and optionally returns a payload; `registerFlightDataSource(id, hook)` registers further, named collectors beside it. The handler envelopes what the hooks returned as `{ value, data }`, with `data` **keyed by source id** — `{ [source]: slice }`, the unnamed hook’s slice under the reserved id `"true"` — under the `X-Single-Flight` response header, whose value is the list of sources actually folded. It runs after `transformResult`, only for scripted calls that sent the header on the request; redirect-with-data works because the outcome’s `response` carries `Location`, so the hook can produce data for the destination route.
- **Client:** `subscribeFlightData(consumer)` registers the unnamed consumer; `subscribeFlightData(id, consumer)` a named one. On a single-flight response the transport decodes `{ value, data }` and delivers **each registered consumer its own slice** (`data[source]`, with the response as envelope context), in registration order, awaiting async consumers so caches are seeded before the caller sees the value; a consumer whose source the header does not name is skipped, unless the response carries integration metadata (a redirect, `X-Revalidate`), in which case it runs with `undefined` to apply the metadata. `value` returns to the caller as if the call were plain. This delivery is one implementation shared by every transport that can carry the envelope — the plain body and the frames transport’s flight stream — so a mutation reads identically over either. One active consumer per source; with none registered, the response passes through whole for the integration to `decodeResponse` itself.
...
| ---------------------- | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| **Declaration-static** | properties of the function itself                      | `GET(fn)` for method; `live(fn)` for lifetime; `withMeta(fn, meta)` for user-declared transport metadata |
...
Calls go over HTTP GET with arguments codec-encoded in the query string of the call’s data address — cacheable by HTTP infrastructure (a read sends no header of its own, so caches and `<link rel="preload" as="fetch">` match it on the URL alone, and the data address serves the codec shape to every caller, so what a cache stores there is right for anyone who reads it). Arguments too long for a url dispatch over POST instead, which costs the cache entry rather than meeting whichever proxy in the chain draws the line at a 414. Cache headers flow through the handler’s existing header forwarding: `respond(data, { headers: { "cache-control": "max-age=60" } })`. `serverFunctionUrl(fn, ...args)` renders that url as a value — for `<link rel="preload" as="fetch" crossorigin href={serverFunctionUrl(getUser, id)} />`, a prefetch, a service-worker warm, or a fetch by hand (decode the answer with `decodeResponse`) — built the way the transport builds it, from the reference (the only thing that knows the call is a GET). It refuses, rather than answering with an address nothing would request, for a reference on the default transport (a POST is not described by its url; `invoke(fn, { priority: "low" }, ...args)` starts such a call early), for arguments that need the codec, and for a url past the POST-fallback length. Server-side, the wrapper is identity-flavored — SSR calls stay in-process. Because function-level directives round-trip wrapper calls (above), this needs **no compiler involvement**.

Conditional reads belong here too, and to the **browser**: set `ETag`/`Cache-Control` on a GET-declared read and the browser owns the conditional exchange — it sends `If-None-Match`, receives the 304, and replays its cached 200 without the caller ever seeing a 304. Answering a _scripted_ call with a hand-rolled `respond(undefined, { status: 304 })` answers a question the transport never asked — it sends no conditional headers — so the bodiless answer resolves the call to `undefined`, which reads as data loss. The dev build warns when a scripted call is answered with a 304 (#3101).
...
#### `live(fn)`: a standing answer, for the response's lifetime (shipped over the top-level iterable; extended 2026-09-22/23, extension unbuilt — Stage 8 Phase A, plan in `documentation/plans/stage8-connection-transport.md`)

`live` declares that a server function's answer is a **standing answer**: the current state of one logical query, which the source re-yields on any invocation, and which the caller wants to keep receiving for as long as it is reading. It is a _client_ declaration — the server half is identity-flavored, in-process SSR calls have no wire — and it composes outermost, because its behavior wraps the call: `live(GET(fn))`.

```ts
import { live, GET } from "@solidjs/web/server-functions";

export const price = live(
...
const src = price("ACME"); // the reconnecting iterable itself, synchronously
src.onstatus = setStatus; // optional: "connected" | "reconnecting" | "closed"
const current = createMemo(() => src);
```

**What is shipped.** Calling a live reference returns an async iterable that survives the connection. Each iteration is its own connection; post-connect deaths re-invoke with exponential backoff (reset per healthy value, woken early by connectivity returning); a failure on the _first_ connect rejects like a normal call (a typo must not retry silently); definite rejections (4xx) fail fast, except the statuses that themselves say "retry" (408, 425, 429) and any answer carrying `Retry-After`; `break` aborts the in-flight request, and a caller-supplied signal through `invoke` ends the iteration for good, across reconnects. `onstatus` on the returned iterable is the one connection-state surface — `"connected"` per successful (re)connect, `"reconnecting"` with the error per post-connect death, `"closed"` on completion or consumer end. Live calls are reads and never request single-flight enveloping. The declaration is deliberately wire-level only: no cached value, no sharing — the value-shaped contract makes the _server_ the cache, and sharing is the reactive graph's (hoist the memo) or a data layer's (a router's live query holds one iteration and multicasts it). During SSR a live source read by a memo or projection is **hybrid** by default: the render takes the first value into markup and closes the iterator; the client's `live` re-invokes after hydration. One value per source crosses the document; the connection is the client's.

**The extension: the lifetime is the response's, not the top-level iterable's.** As shipped, `live` watches the iterable the function returns. An answer that is an _object_ whose properties are promises or async iterables — `{ meta, progress: gen }` — is a one-value stream to it: the nested streams settle over the same connection, but their deaths are invisible to the loop and their SSR handoff never happens (a nested unbounded iterable holds the document open). The extension makes the declaration mean what it says. **`live` claims the whole response.**

- **Lifetime.** A live connection is alive while its response is: until every deferred in the answer, nested ones included, has completed and the stream carries its terminal record. A body that ends _without_ the terminal record is a **death**; one that carries it is a **completion**. Death reconnects; completion completes the iterable.
- **Reconnect re-yields the whole answer.** The iterable yields once per connection: the function's answer, fresh — a new object with new nested streams. There is no continuation of a nested iterator across connections (nothing the client holds can be resumed on a server that does not hold it); re-derivation is the baseline and cursors are an opt-in above it. A consumer over a projection re-reconciles; a memo sees a new identity. Value-shaped sources — the shipped case — are unchanged: the whole answer _is_ the value.
- **SSR handoff at first settlement.** In-process, the server half brands every nested async source of the answer as live (a walk over the result, the same one the encoder makes); each takes its first value into markup and closes, the response completes as any response does, and the client re-invokes when the source's own hydration scope releases — the root pass's release for shell content, the boundary's own release for streamed content — not when the whole page has hydrated. (The shipped takeover gate waits for page-wide hydration end; that contradicts the per-boundary snapshot design and is fixed as part of this work — a behavior change, flagged.) "First settlement" is nothing new: it is the response completing after every source took one value.
- **Undeclared death is an error.** An undeclared streaming answer that dies mid-stream rejects the consumer's pull like any transport failure — `<Errored>` catches it, nothing resumes. `live` is the only reconnect in the runtime; nothing re-invokes on its own.
- **Framing.** Each live source is its own connection, as today — subscribing is a request, unsubscribing is closing it (which is also how the server learns to tear the render down), and nothing on the page coordinates with anything else — but the response is framed as server-sent events: `Content-Type: text/event-stream`, `Cache-Control: no-store`, `X-Accel-Buffering: no`, the codec's payload strings one per `data:` event (multi-line payloads split and reassembled per the SSE spec — the one place a real bug hides, so it is tested), a comment-line heartbeat every 20s against proxy idle timeouts. Nothing else differs from today's streamed answer on the wire — same connection, same timeouts — but the content type is one middleboxes are trained to pass through unbuffered and uncompressed (the reason SvelteKit moved `query.live` off ndjson). `EventSource` is not used; the loop reads with `fetch` and keeps headers, `POST` fallback, `prepareRequest`, and abort. The precondition is a backend that holds connections, over HTTP/2 (six connections per origin under HTTP/1.1; the Vite dev server speaks HTTP/2 with `server.https`, and dev warns past five live connections without it) — the precondition LiveView, Datastar, and `query.live` all share, and there is nothing to configure. A platform that kills a response at a ceiling kills each live response there; the loop reads it as a death, reconnects with its position, and `onstatus` shows the cycle — `query.live`'s behavior on the same platform. A buffering proxy that ignores `X-Accel-Buffering` is silent until the stream dies. Both are the deployment's to fix. Nothing an app wrote against today's `live` changes: a live call _is_ a direct call, so every client and server hook applies as it does to one.
- **Position.** Every reconnect carries `Last-Event-ID`: a source with cursors names its own position; a value-shaped source without one is given the server's digest of the last value it yielded, and a source whose current value digests the same emits nothing on reconnect. Never as an argument — a position is not identity.
- **Hidden pages pause.** A page that stays hidden past a grace window (on the order of 30s, so a tab flick costs nothing) has every live connection closed; when the page is visible again the loop reconnects, with its position, and the reconnect is conditional like any other. A pause is not a death: the close fires no status — the last status holds through the pause — and the return reconnect fires `"connected"` when it lands, or enters the ordinary backoff (`"reconnecting"`, with the error) when it does not. What a background tab costs is one reconnect on return; what it holds meanwhile is nothing. (Datastar's default; a behavior change to shipped `live`, flagged.) The one case this is wrong for is a source whose job is to be heard while the tab is in the background — notifications, a call ringing. Not built; a declaration-level opt-out is future, if something asks for it.

**Server components.** A server component is a server function whose answer is a component ([RFC 11](11-server-components.md)); its render is a stream of markup over the same connection. That makes it a nested-async answer in the sense above, and `live` covers it with no further vocabulary:

```tsx
export const feed = live(
  GET(async (room: string) => {
    "use server";
    const messages = subscribeRoom(room); // raw async iterable; the component reads it
    return (props: { compose: Slot }) => <Feed messages={messages} compose={props.compose} />;
  })
);
...
**Routers.** A data layer may apply `live` (and `GET`) on the author's behalf — a live query is "`live` + shared" — but the core spelling is the documented one, and the reliable site for `GET` is the definition: the has-method grant arms when the module that applies `GET` is loaded on the server, and the function's own module always is. A `GET` applied at a consumer site arms only if that module is also in the server graph — the topology caveat — and a layer that applies declarations on the author's behalf owns it.

**Public API:** no new export on the client, nothing to configure on the server. Three behavior changes to a shipped export, flagged: `live` over nested-async answers claims the response; the post-hydration takeover fires per scope instead of at page-wide hydration end; `live` pauses on hidden pages. The event-stream framing, `Last-Event-ID`, and the terminal record (on both framings) are new wire. Withdrawn unbuilt: `SSE(fn)`, `enableEventStream()`, `Accept: text/event-stream` as a client declaration, framing-follows-method, a per-page channel, and `live: { transport, hold }` server configuration (see Alternatives considered).
...
- **A live transport decision (2026-09-22/23)** — argued through in one night and dissolved. A shared per-page event-stream channel carrying every live source would fix the connection count, but an SSE response is a fixed set at request time: adding a source to an open stream needs the add to reach the instance holding it and thread a new iteration into a writer another request owns (a mailbox on the channel handler; an adapter across instances) — a subscription protocol with a socket's requirements and none of its carrier, fighting a client where every subscription is fine-grained. A `transport: "polling" | "sse"` enum with a server-side default was a distinction without a difference: the "poll" was the same seroval stream ended at a deadline, and the deadline (`hold`) was a ramp for platforms outside the feature's precondition. What survives is a framing note on `live` and nothing to configure. `hold` (one optional number that cycles live responses under a ceiling) and a socket carrier (where subscribe/unsubscribe are native) are recorded as future if asked for. Also withdrawn unbuilt on the way: `SSE(fn)`, `enableEventStream()`, `Accept: text/event-stream` as a client declaration, and a rule that framing follows the request method.
