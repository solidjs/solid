...
describe("patch-mode list driver", () => {
  ...
  test("shallow store list: reference semantics — replaced records rebuild, moved references retain", () => {
    createRoot(dispose => {
      let div!: HTMLDivElement;
      // Shallow contract: children are served RAW, so the store IS the array.
      // IDENTITY RULING: with no `keyed` declared, the view's identity is the
      // RECORD REFERENCE — the driver must match classic mapArray exactly: a
      // key-aligned slot replaced by a fresh record REBUILDS its row (the
      // reconcile key is a store-write detail the view never declared).
      const r1 = make(1)[0];
      const r2 = make(2)[0];
      const r3 = make(3)[0];
      const [shRows, setState] = createStore([r1, r2, r3], { shallow: true } as any);
      <div ref={div}>
      ...
      // Aligned tick: same keys, row 2 replaced BY REFERENCE — reference
      // semantics rebuild that row; same-reference slots keep their nodes.
      setState(s => {
        reconcile([r1, { id: 2, label: "X2" }, r3], "id")(s);
      });
      flush();
      expect(labels(div)).toBe("L1,X2,L3");
      expect(rows(div)[0]).toBe(tr1);
      expect(rows(div)[1]).not.toBe(tr2);
      expect(tr2.isConnected).toBe(false);
      expect(rows(div)[2]).toBe(tr3);

      // Structure: reorder + remove + add — SAME references move with their
      // nodes; the new record binds fresh.
      const r4 = { id: 4, label: "L4" };
      setState(s => {
        reconcile([r3, r4, r1], "id")(s);
      });
      flush();
      ...
      // Retained-slot replacement THROUGH a structural op: key 3 keeps its
      // position but the record is fresh — rebuilt, not patched in place.
      const tr3b = rows(div)[0];
      setState(s => {
        reconcile([{ id: 3, label: "Z3" }, r4, r1], "id")(s);
      });
      flush();
      expect(labels(div)).toBe("Z3,L4,L1");
      expect(rows(div)[0]).not.toBe(tr3b);
      expect(rows(div)[2]).toBe(tr1);
...
// External-audit regression coverage (2026-08-24): two findings against the
// landed driver surface, independent of the (rejected-for-now) keyed proposal.
describe("patch-mode list driver — audit regressions", () => {
  test("two driven lists over ONE shallow array both receive slot ticks and structure", () => {
    createRoot(dispose => {
      let divA!: HTMLDivElement;
      let divB!: HTMLDivElement;
      ...
      expect(labels(divA)).toBe("L1,L2,L3");
      expect(labels(divB)).toBe("L1,L2,L3");

      // Slot tick (same keys, one record replaced): BOTH lists must update —
      // slot registration was single-consumer before the audit fix (the
      // second registration silently overwrote the first).
      setState(s => {
        reconcile([make(1)[0], { id: 2, label: "X2" }, make(3)[0]], "id")(s);
      });
      flush();
      expect(labels(divA)).toBe("L1,X2,L3");
      expect(labels(divB)).toBe("L1,X2,L3");

      // Structure: both lists apply row ops.
      setState(s => {
        reconcile([make(3)[0], make(1)[0]], "id")(s);
      });
      flush();
      expect(labels(divA)).toBe("L3,L1");
      expect(labels(divB)).toBe("L3,L1");

      // Unbind one list (dispose order: A's region cleared manually is not
      // trivial here, so assert via full dispose at the end instead — the
      // splice-unbind path is exercised by the removal in applyOps above).
      dispose();
    });
  });

  test("optimistic family arrays DECLINE the driver — structural optimism renders classically", () => {
    createRoot(dispose => {
      let div!: HTMLDivElement;
      const [rows] = createOptimisticStore<{ list: { id: number; label: string }[] }>({
        list: [
          { id: 1, label: "L1" },
          { id: 2, label: "L2" }
        ]
      ...
      <div ref={div}>
        <For each={rows.list}>{pureRow}</For>
      </div>;
      // Family structural changes emit no row/slot ops and the proxy identity
